...
<%@ attribute name="uploadPathLocked" type="java.lang.Boolean" required="false" description="锁定文件的上传路径"%>
<%@ attribute name="uploadPath" type="java.lang.String" required="false" description="打开文件管理的上传路径"%>
...
<script type="text/javascript">
	function ${input}FinderOpen(){
		...
		var start = "<c:if test='${uploadPathLocked}'>${ctype}:${uploadPath}/</c:if>";
		var date = new Date(), year = date.getFullYear(), month = (date.getMonth()+1)>9?date.getMonth()+1:"0"+(date.getMonth()+1);
		var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&start=" + start +
